...
class QuizDisplay extends Renderer {
  ...
  _generateQuestion() {
    ...
    return `
      <form class="grid-row question-form">
        ...
        <label class="grid-item col-3 question-text" for="question-choices">${
          this.model.currQuestion().text
        }</label>
        ${answerElem}
  ...
  _generateReviewCorrect() {
    return `
      <div class="grid-row col-3">
        <h1 class= "grid-item col-3 question-text">${
          this.model.currQuestion().text
        }</h1>
  ...
  template() {
    ...
    if (this.model.active && this.model.currQuestion().answerStatus() === -1) {
      ...
      this.model.active &&
      this.model.currQuestion().answerStatus() === 0
    ) {
      return this._generateReviewIncorrect();
    } else if (
      this.model.active &&
      this.model.currQuestion().answerStatus() === 1
    ) {
      return this._generateReviewCorrect();
    } else if (
      !this.model.active &&
      this.model.asked.length &&
      !this.model.unasked.length
    ) {
